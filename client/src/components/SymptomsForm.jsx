...
const SymptomsForm = () => {
    ...
    const handleChange = (event) => {
        ...
        if (value) {
            const filteredSymptoms = Object.keys(symptoms_dict).filter(symptom =>
    ...
    return (
        <div className="form-container">
            <h2 className="form-heading">Disease Forecasting & Medical Recommendations</h2>
            <form onSubmit={handleSubmit} className="symptom-form">
                ...
                <button type="submit" className="submit-button">Submit Symptoms</button>
            </form>

            {apiResponse && (
                <div className="response-container">
                    <h3 className="response-heading">Predicted Disease: {apiResponse.predicted_disease}</h3>
                    <p><strong>Description:</strong> {apiResponse.description}</p>
                    <h4>Diets:</h4>
                    <ul>
                        {apiResponse.diets.map((diet, index) => (
                            <li key={index}>{diet.replace(/'/g, '')}</li>
                        ))}
                    </ul>
                    <h4>Medications:</h4>
                    <ul>
                        {apiResponse.medications.map((medication, index) => (
                            <li key={index}>{medication.replace(/'/g, '')}</li>
                        ))}
                    </ul>
                    <h4>Precautions:</h4>
                    <ul>
                        {apiResponse.precautions.map((precaution, index) => (
                            <li key={index}>{precaution}</li>
                        ))}
                    </ul>
                    <h4>Workout Suggestions:</h4>
                    <ul>
                        {apiResponse.workout.map((workout, index) => (
